...
class OfflineQueue(private val context: Context) {
    ...
    fun flush(): Int {
        ...
        for (item in queue) {
            when (val r = ApiClient.send(item)) {
                is ApiResult.Success     -> { remove.add(item); sent++ }
                is ApiResult.ServerError -> { remove.add(item) }  // bad payload — drop
                is ApiResult.NetworkError -> break                  // still offline
            }
        }
    ...
    @Synchronized fun size(): Int    = loadQueue().size
